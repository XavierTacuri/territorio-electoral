...
test('Territorio IA produce panorama y debate factual trazables para Gualaceo', async ({
  request,
}) => {
  const token =
    process.env.E2E_TOKEN ||
    (await apiToken(request, process.env.E2E_USER_NAME || e2eUsers.manager));
  const headers = { Authorization: `Bearer ${token}` };
  const campaigns = await (
    await request.get('/api/v1/campaigns?page_size=100', { headers })
  ).json();
  const campaign = campaigns.items.find(
    (item: { slug: string; name: string }) => item.slug === 'gualaceo-e2e-2027',
  );
  ...
  expect([
    ...new Set(
      panoramaBody.citations.map((item: { evidence_class: string }) => item.evidence_class),
    ),
  ]).toEqual(expect.arrayContaining(['OFFICIAL', 'CAMPAIGN']));
  expect(
    panoramaBody.citations.some(
      (item: { source_type: string }) => item.source_type === 'CITIZEN_NEED',
    ),
  ).toBe(true);
  ...
  const debate = await request.post(`/api/v1/campaigns/${campaign.id}/territory-ai/query`, {
    ...
    data: {
      question:
        'Prepárame un resumen factual de la evidencia disponible sobre vialidad para un debate.',
    },
  });
