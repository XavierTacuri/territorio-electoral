...
test('cambio de contraseña desde Mi cuenta revoca sesión y permite restauración legítima', async ({
  page,
  request,
}) => {
  ...
  await expect(
    page.getByText('Contraseña actualizada correctamente. Inicia sesión nuevamente.'),
  ).toBeVisible();
  ...
  expect(
    (
      await request.post('/api/v1/auth/login', {
        form: { username: e2eUsers.candidate, password: originalPassword },
      })
    ).status(),
  ).toBe(401);
  expect(
    (
      await request.post('/api/v1/auth/login', {
        form: { username: e2eUsers.candidate, password: temporaryPassword },
      })
    ).status(),
  ).toBe(200);
...
test('ADMIN navega a configuración IA segura y prueba FakeProvider', async ({ page, request }) => {
  ...
  expect(JSON.stringify(body).toLowerCase()).not.toMatch(
    /authorization|secret|openai_api_key|bearer|hash/,
  );
});
...
test('usuario no ADMIN no ve ni accede a configuración IA', async ({ page, request }) => {
  ...
  expect((await request.get('/api/v1/admin/ai-provider', { headers: auth(token) })).status()).toBe(
    403,
  );
  expect(
    (await request.post('/api/v1/admin/ai-provider/test', { headers: auth(token) })).status(),
  ).toBe(403);
});
