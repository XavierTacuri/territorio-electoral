...
test('Candidate consulta el Centro de Comando y sus módulos', async ({ page, request }) => {
  ...
  for (const value of ['34.784', '71,00 %', '24.697', '68,58 %', '71,89 %'])
  ...
  const boundaries = await request.get(
    `/api/v1/campaigns/${campaign.id}/map/boundaries?level=PARISH&limit=5000`,
    { headers: { Authorization: `Bearer ${candidateToken}` } },
  );
  expect(boundaries.status()).toBe(200);
  expect((await boundaries.json()).features).toHaveLength(9);
  await expect(page.getByRole('link', { name: 'VER PANORAMA ELECTORAL' })).toHaveAttribute(
    'href',
    `/app/campaigns/${campaign.id}/panorama`,
  );
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.screenshot({
    path: '../quality-artifacts/command-center-candidate-desktop.png',
    fullPage: true,
  });
...
test('Centro de Comando no produce overflow en móvil', async ({ page, request }) => {
  ...
  await page.screenshot({
    path: '../quality-artifacts/command-center-candidate-mobile-375.png',
    fullPage: true,
  });
