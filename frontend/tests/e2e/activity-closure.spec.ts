...
async function e2eCampaign(request: Parameters<typeof apiToken>[0], token: string) {
  ...
  const campaign =
    campaigns.find((item) => item.slug === 'gualaceo-e2e-2027') ??
    campaigns.find((item) => item.slug === 'territorio-sintetico-e2e');
...
async function prepareActivity(
  ...
  const parishResponse = await request.get(
    `/api/v1/parishes${campaign.canton_id ? `?canton_id=${campaign.canton_id}` : ''}`,
    {
      headers: { Authorization: `Bearer ${token}` },
    },
  );
...
test('CANDIDATE completa una actividad con necesidad detectada (el cierre ya no crea seguimientos)', async ({
  page,
  request,
}) => {
  const managerToken = await apiToken(request, e2eUsers.manager);
  const { campaign, activity } = await prepareActivity(
    request,
    managerToken,
    uniqueE2eValue('[E2E] Actividad cierre completa'),
  );
  ...
  const row = page
    .getByRole('row')
    .filter({ hasText: activity.id })
    .or(
      page.getByRole('row').filter({ hasText: uniqueE2eValue('[E2E] Actividad cierre completa') }),
    );
  ...
  const summary =
    'Se realizó la actividad territorial programada y se registraron los principales resultados del encuentro.';
  await page.getByLabel('Resumen de la actividad').fill(summary);
  await page
    .getByLabel('Resultados u observaciones')
    .fill('Observaciones sintéticas del encuentro E2E.');
  await expect(page.getByLabel('Prioridad')).toHaveCount(0);
  await expect(page.getByLabel('Fecha límite')).toHaveCount(0);
  await page
    .getByLabel('Necesidad detectada (opcional)')
    .fill(uniqueE2eValue('[E2E] Necesidad de cierre'));
  await page
    .getByLabel('Descripción de la necesidad')
    .fill('Necesidad sintética creada durante el cierre E2E.');
  ...
  await page
    .getByRole('row')
    .filter({ hasText: uniqueE2eValue('[E2E] Actividad cierre completa') })
...
test('CANDIDATE suspende con motivo y no expone el cierre estructurado', async ({
  page,
  request,
}) => {
  const managerToken = await apiToken(request, e2eUsers.manager);
  const { campaign, activity } = await prepareActivity(
    request,
    managerToken,
    uniqueE2eValue('[E2E] Actividad cancelada'),
  );
  await browserLogin(page, e2eUsers.candidate);
  await page.goto(`/app/campaigns/${campaign.id}/activities`);
  const row = page
    .getByRole('row')
    .filter({ hasText: uniqueE2eValue('[E2E] Actividad cancelada') });
  await row.getByRole('button', { name: 'Editar actividades' }).click();
  const editDialog = page.getByRole('dialog', { name: 'Editar actividad' });
  await editDialog.getByLabel('Estado').click();
  ...
  await expect(
    page.getByRole('row').filter({ hasText: uniqueE2eValue('[E2E] Actividad cancelada') }),
  ).toContainText('Suspendida');
  const detailResponse = await request.get(
    `/api/v1/campaigns/${campaign.id}/activities/${activity.id}`,
    { headers: { Authorization: `Bearer ${managerToken}` } },
  );
