  ...
  return (
    <Stack spacing={2.5}>
      <Box>
        ...
        {preview.subtitle && (
          <Typography color="text.secondary">{preview.subtitle}</Typography>
        )}
      ...
      <Alert severity="info" variant="outlined">
        Informe descriptivo. No constituye predicción electoral, persuasión política ni perfilamiento individual.
      </Alert>
      ...
      <Card variant="outlined" sx={{ bgcolor: 'action.hover' }}>
        <CardContent>
          <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
            ...
            <Chip size="small" label={preview.narrative.provider === 'deterministic-fallback' ? 'Redacción del sistema' : 'Redactado por IA grounded'} />
          </Stack>
                ...
                >
                  <Box sx={{ minWidth: 0 }}>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {SOURCE_TYPE_LABELS[citation.source_type] ?? citation.source_type} · {citation.source_name}
                      {citation.record_date ? ` · ${citation.record_date}` : ''}
                    </Typography>
                  </Box>
                  <Stack direction="row" spacing={1} alignItems="center">
                    <Chip size="small" color={evidenceChipColor(citation.evidence_class)} label={EVIDENCE_CLASS_LABELS[citation.evidence_class] ?? citation.evidence_class} />
                    {citation.deep_link && (
                      <MuiLink component={RouterLink} to={citation.deep_link} sx={{ whiteSpace: 'nowrap' }}>
                        Ver origen
                      </MuiLink>
                    )}
                    {!citation.deep_link && citation.source_url && (
                      <MuiLink href={citation.source_url} target="_blank" rel="noreferrer" sx={{ whiteSpace: 'nowrap' }}>
                        Ver fuente
                      </MuiLink>
                    )}
                  </Stack>
