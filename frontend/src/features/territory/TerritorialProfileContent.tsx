...
import { needStatusLabel, DEMO_DATA_NOTICE } from '../../lib/labels';
...
export function TerritorialProfileContent({
  campaignId,
  parishId,
}: {
  campaignId: string;
    ...
    <>
      ...
      <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
        <Grid container spacing={2}>
          <Grid size={{ xs: 6, sm: 4, md: 2.4 }}>
            <KpiMetric label="Electores actuales" value={selected.registered_voters_current} />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2.4 }}>
            <KpiMetric
              label="Votantes esperados"
              value={selected.projection?.expected_voters_central ?? null}
            />
          </Grid>
          <Grid size={{ xs: 6, sm: 4, md: 2.4 }}>
      ...
      <Grid container spacing={2}>
        <Grid size={{ xs: 12, lg: 5 }}>
          <Card title="PADRÓN ELECTORAL" eyebrow="Datos oficiales del CNE">
            <Grid container spacing={2}>
              <Grid size={12}>
                <Metric
        ...
        <Grid size={{ xs: 12, lg: 7 }}>
          <Card title="PARTICIPACIÓN ELECTORAL" eyebrow="Estimación de participación">
            <Grid container spacing={2}>
              {(
                [
                  ['low', selected.projection?.low, selected.projection?.expected_voters_low],
                  [
                    'central',
                    selected.projection?.central,
                    selected.projection?.expected_voters_central,
                  ],
                  ['high', selected.projection?.high, selected.projection?.expected_voters_high],
                ] as const
              ).map(([key, rate, voters]) => (
                <Grid key={key} size={4}>
                  <Metric
                    label={participationScenarioLabels[key]}
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
              Estimación basada en elecciones anteriores
            </Typography>
          </Card>
        </Grid>
        <Grid size={{ xs: 12, lg: 7 }}>
          <Card title="ANTECEDENTES ELECTORALES" eyebrow="Datos oficiales del CNE">
            {!hasHistory ? (
              <Alert severity="info">
