...
const activity = (approval_status: Activity['approval_status']) =>
  ({ approval_status, status: 'PLANNED' }) as Activity;

describe('acciones del detalle de actividad', () => {
  it.each(['CANDIDATE', 'CAMPAIGN_MANAGER'])('%s aprueba y rechaza pendientes', (role) => {
    expect(activityDetailActions(user(role), activity('PENDING_APPROVAL'))).toMatchObject({
      approve: true,
      reject: true,
      execution: false,
    });
  ...
  it('coordinador no aprueba pendientes y corrige rechazadas', () => {
    expect(
      activityDetailActions(user('TERRITORIAL_COORDINATOR'), activity('PENDING_APPROVAL')).approve,
    ).toBe(false);
    expect(
      activityDetailActions(user('TERRITORIAL_COORDINATOR'), activity('REJECTED'))
        .correctAndResubmit,
    ).toBe(true);
  });
