...
vi.mock('../features/dashboard/CommandCenterMap', () => ({
  CommandCenterMap: () => (
    <div role="region" aria-label="Mapa operativo territorial">
...
const parishes = Array.from({ length: 9 }, (_, index) => ({
  parish_id: index + 1,
  name: index ? `Parroquia ${index + 1}` : 'Gualaceo',
  dpa_code: `01035${index}`,
  registered_voters_current: index ? 1700 : 21184,
...
const analysis = {
  ...
  snapshot: { snapshot_date: '2026-07-16', registered_voters: 34784 },
  historical: { '2019': { turnout_rate: 0.6858 }, '2023': { turnout_rate: 0.7189 } },
  projection: { model_code: 'TURNOUT_HISTORICAL_WEIGHTED_V1', expected_voters_central: 24697 },
  parishes,
};
const operations = {
  activities: { total: 8, demo: 4, upcoming: 3, pending_approval: 2, completed: 4 },
  needs: { total: 5, demo: 2, open: 4, under_review: 1, validated: 2 },
  commitments: { pending: 2, in_progress: 1, overdue: 1, completed: 4 },
  coverage: { total_parishes: 9, with_activities: 7, with_needs: 4, with_commitments: 3 },
  can_approve: true,
};
const agenda = {
  activities: [{ id: 'a1', title: 'Visita', activity_date: new Date().toISOString().slice(0, 10) }],
  pending_approval: [{ id: 'a2' }, { id: 'a3' }],
  commitments: [{ id: 'c1' }],
};
const territory = {
  parishes: [
    {
      ...
      latest_activities: [
        {
          id: 'a1',
          title: '[DEMO] Reunión territorial en Jadán',
          date: '2026-08-30',
          status: 'COMPLETED',
        },
      ],
...
const published = {
  items: [
    {
      id: 's1',
      name: '[DEMO] Encuesta general',
      status: 'PUBLISHED',
      fieldwork_start_date: '2026-08-15',
      fieldwork_end_date: '2026-08-18',
      sample_size_total: 800,
    },
  ],
  total: 1,
  page: 1,
...
const handlers = [
  ...
  http.get('*/api/v1/campaigns/campaign-1', () =>
    HttpResponse.json({
      id: 'campaign-1',
      name: 'Gualaceo2026',
      canton_name: 'Gualaceo',
...
function renderDashboard(studyData: typeof published | 'error' = published) {
  ...
  client.setQueryData(['campaign', 'campaign-1'], {
    id: 'campaign-1',
    name: 'Gualaceo2026',
    canton_name: 'Gualaceo',
    province_name: 'Azuay',
    ...
    <QueryClientProvider client={client}>
      <MemoryRouter initialEntries={['/app/campaigns/campaign-1/dashboard']}>
        <Routes>
          ...
          <Route path="/app/campaigns/:campaignId/territory-ai" element={<div>IA abierta</div>} />
        </Routes>
      </MemoryRouter>
    </QueryClientProvider>,
  );
...
describe('Centro de Comando Territorial', () => {
  ...
  it('mantiene el Dashboard cuando una fuente secundaria no tiene datos', async () => {
    ...
    expect(
      screen.getByText('Aún no existen encuestas publicadas para esta campaña.'),
    ).toBeVisible();
  });
  ...
  it('muestra empty state de encuestas', async () => {
    renderDashboard({ ...published, items: [], total: 0 });
    expect(
      await screen.findByText('Aún no existen encuestas publicadas para esta campaña.'),
    ).toBeVisible();
  });
  ...
  it('abre Territorio IA con la pregunta sugerida', async () => {
    ...
    const suggestion = await screen.findByRole('button', {
      name: '¿Cuál es el panorama electoral actual?',
    });
