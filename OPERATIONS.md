...
- Con `ARTIFACT_STORAGE_PROVIDER=s3` (Fase 4A/4B, AWS): reportes y evidencia/actas viven en el bucket S3 configurado, no en un volumen de este host — el backup de esos artefactos es responsabilidad del propio bucket (versioning + replicación, ver `docs/aws/PRODUCTION_ARCHITECTURE.md`), no de `evidence_$(date).tgz`. La base de datos sigue siendo la única fuente de verdad de qué `storage_key` existe, su tamaño y su sha256 — el bucket nunca sustituye ese rol.
