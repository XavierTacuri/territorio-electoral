...
class DashboardService:
    ...
    def overview(self, campaign_id: UUID, user: User, filters: DashboardFilters):
        ...
        metric_specs = [
            ...
            ("pending_commitments", "Compromisos pendientes", commitments["pending"]), ("in_progress_commitments", "Compromisos en progreso", commitments["in_progress"]), ("completed_commitments", "Compromisos completados", commitments["completed"]), ("overdue_commitments", "Compromisos vencidos", commitments["overdue"]),
        ...
        summary = f'Durante el período se registraron {current["completed"]} actividades completadas en {current["covered"]} parroquias. Se contabilizaron {current["attendees"]} asistentes estimados, {current["needs"]} necesidades activas y {commitments["overdue"]} compromisos vencidos.'
    ...
    def data_quality(self,campaign_id,user,filters):
        ...
        commitments=self.repo.commitment_counts(campaign.id,period.date_to,scope.parish_ids);add("COMMITMENTS_WITHOUT_RESPONSIBLE","Compromisos sin responsable",commitments["without_responsible"],"COMMITMENTS","Asignar un responsable.");add("COMMITMENTS_WITHOUT_DUE_DATE","Compromisos sin fecha límite",commitments["without_due_date"],"COMMITMENTS")
