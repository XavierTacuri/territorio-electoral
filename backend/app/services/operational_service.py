...
class OperationalService:
    ...
    def summary(self,campaign_id,user,date_from=None,date_to=None):
        ...
        return OperationalSummaryRead(date_from=date_from,date_to=date_to,completed_activities=completed,planned_activities=sum(a.status=="PLANNED" for a in acts),cancelled_activities=sum(a.status=="CANCELLED" for a in acts),estimated_attendees=sum(x.estimated_attendees for x in summaries),activities_by_type=bytype,activities_by_parish=byparish,top_needs=top,needs_by_parish=needsbyparish,commitments=CommitmentStatusSummary(pending=counts["PENDING"],in_progress=counts["IN_PROGRESS"],completed=counts["COMPLETED"],cancelled=counts["CANCELLED"],overdue=over),uncovered_parishes=uncovered,summary_text=text)
