...
class MapService:
 ...
 def bounds(self,campaign_id,user,filters):
  # Placeholder geometry must never drive the viewport: only OFFICIAL_IMPORT
  # rows are eligible for canton/parish bounding boxes.
  campaign,_,scope,_,_=self.context(campaign_id,user,filters);canton=self.db.get(Canton,campaign.canton_id);canton_bbox=self.repo.extent(Canton.geometry,[Canton.id==canton.id,Canton.geometry_source=='OFFICIAL_IMPORT']);accessible=self.repo.extent(Parish.geometry,[Parish.id.in_(scope.parish_ids),Parish.geometry_source=='OFFICIAL_IMPORT'])
 ...
 def boundaries(self,campaign_id,user,filters,level="PARISH",include_metrics=False,metric=None):
  ...
  for obj,geometry in rows:
   # Real provenance from the row itself — never assume OFFICIAL_IMPORT.
   props={"resource_id":str(obj.id),"resource_type":level,"name":obj.name,"code":obj.dpa_code,"territory_level":level,"campaign_id":str(campaign.id),"geometry_source":obj.geometry_source,"geometry_quality":obj.geometry_quality}
