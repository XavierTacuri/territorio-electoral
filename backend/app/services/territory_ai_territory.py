...
class TerritoryAIResolver:
    ...
    def _canton(self,campaign,canton):return TerritoryReference(id=canton.id,name=canton.name,dpa_code=canton.dpa_code,level="CANTON",campaign_id=campaign.id,canton_id=canton.id)
    def _parish(self,campaign,parish):return TerritoryReference(id=parish.id,name=parish.name,dpa_code=parish.dpa_code,level="PARISH",campaign_id=campaign.id,canton_id=parish.canton_id)
