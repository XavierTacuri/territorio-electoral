...
class StudyType(StrEnum): POLL="POLL"; TRACKING_POLL="TRACKING_POLL"; EXIT_POLL="EXIT_POLL"; OTHER="OTHER"
...
class StudyBase(BaseModel):
    ...
    source_type: str; source_url: HttpUrl|None=None; notes: str|None=None; is_official: bool=False
    ...
    def valid(self):
        ...
        if self.study_type==StudyType.EXIT_POLL and not self.election_process_id: raise ValueError("Exit poll requiere proceso electoral")
        return self
...
class StudyUpdate(BaseModel):
    ...
    pollster_name:str|None=None; sponsor_name:str|None=None; source_type:str|None=None; source_url:HttpUrl|None=None; notes:str|None=None
...
class OptionInput(BaseModel): code:str; label:str; option_type:OptionType; display_order:int=Field(0,ge=0)
...
class StudyRead(StudyBase):
    id:UUID; campaign_id:UUID; status:StudyStatus; created_by_user_id:UUID; created_at:datetime; updated_at:datetime
...
class SurveyImportSummary(BaseModel):
    ...
    status:str; rows_read:int; rows_valid:int; rows_rejected:int; studies:list[str]; territories:int; options:int
