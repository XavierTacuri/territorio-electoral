...
def current_election_analysis(campaign_id: UUID, user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    observations = db.execute(select(DemographicObservation, DemographicIndicator).join(DemographicIndicator, DemographicIndicator.id == DemographicObservation.demographic_indicator_id).where(DemographicObservation.parish_id.in_(parish_ids), DemographicObservation.reference_year == 2022, DemographicObservation.is_active.is_(True), DemographicObservation.is_official.is_(True))).all()
    demographic = {pid: {} for pid in parish_ids}
    for obs, indicator in observations: demographic.setdefault(obs.parish_id, {})[indicator.code] = float(obs.value)
