...
def test_rejects_bad_sum_and_count(client,db,admin,admin_headers):
    ...
    issues=client.post(f"/api/v1/survey-studies/{sid}/validate",headers=admin_headers).json()["issues"];assert {x["code"] for x in issues}>={"PERCENTAGE_SUM","COUNT_EXCEEDS_SAMPLE"}
...
def test_csv_validate_is_read_only_then_execute(client,db,admin,admin_headers):
    ...
    csv=("study_code,territory_level,parish_dpa,option_code,option_label,option_type,response_count,percentage\n"+f"STUDY_E2E,PARISH,{parish.dpa_code},ALFA,Opción Alfa,CANDIDATE,40,0.40\n"+f"STUDY_E2E,PARISH,{parish.dpa_code},BETA,Opción Beta,CANDIDATE,35,0.35\n"+f"STUDY_E2E,PARISH,{parish.dpa_code},UNDECIDED,Indecisos,UNDECIDED,25,0.25\n").encode()
