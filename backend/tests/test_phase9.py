...
def test_seed_reports_and_alerts_is_idempotent(db):
    assert seed(db)==(10,21);db.commit();assert seed(db)==(10,21);db.commit()
...
def test_template_http_and_alert_evaluation(client,admin_headers,phase9_context):
    campaign,_=phase9_context
    templates=client.get("/api/v1/report-templates",headers=admin_headers);assert templates.status_code==200 and len(templates.json())==10
